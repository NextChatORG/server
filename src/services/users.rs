//! NextChat Server users services module.
//!
...
use crate::{
    core::{
        ...
        security::{encrypt_password, verify_password},
    },
    models::users::{FindQuery, GetAllQuery, SearchQuery, SignUpAndSignInBody, UserDataResponse},
};
...
pub async fn get_all_handler(query: GetAllQuery, client: PgPool) -> Result<impl Reply, Infallible> {
    match sqlx::query("SELECT id, username, profile_image FROM users LIMIT $1 OFFSET $2")
...
/// 4. Invalid query.
pub async fn find_handler(query: FindQuery, client: PgPool) -> Result<impl Reply, Infallible> {
    if query.id.is_some() && query.username.is_some() {
        ...
        // Get the user by id.
        match sqlx::query("SELECT id, username, profile_image FROM users WHERE id = $1")
            .bind(&id)
            .fetch_one(&client)
            .await
        {
        ...
        match sqlx::query("SELECT id, username, profile_image FROM users username = $1")
            .bind(&username)
            .fetch_one(&client)
            .await
        {
...
pub async fn signin_handler(
    ...
    match sqlx::query("SELECT id, password, username, profile_image FROM users WHERE username = $1")
        .bind(&body.username)
        .fetch_one(&client)
        .await
    {
